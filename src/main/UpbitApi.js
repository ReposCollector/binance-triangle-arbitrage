...
const upbit = require('node-upbit-api')();

const UpbitApi = {

    exchangeInfo() {
        return new Promise((resolve, reject) => {
            upbit.exchangeInfo((error, data) => {
    ...
    cloneDepth(ticker, levels) {
        const tmp = upbit.depthCache(ticker);
    ...
    cloneDepths(tickers, levels) {
        return tickers.reduce((clone, ticker) => {
            clone[ticker] = UpbitApi.cloneDepth(ticker, levels);
            return clone;
        }, {});
    },

    marketBuy(ticker, quantity) {
        logger.execution.info(`${upbit.getOption('test') ? 'Test: Buying' : 'Buying'} ${quantity} ${ticker} @ market price`);
        return new Promise((resolve, reject) => {
            upbit.marketBuy(ticker, quantity, (error, response) => {
                if (error) return UpbitApi.handleBuyOrSellError(error, reject);
                if (upbit.getOption('test')) {
    ...
    marketSell(ticker, quantity) {
        logger.execution.info(`${upbit.getOption('test') ? 'Test: Selling' : 'Selling'} ${quantity} ${ticker} @ market price`);
        return new Promise((resolve, reject) => {
            upbit.marketSell(ticker, quantity, (error, response) => {
                if (error) return UpbitApi.handleBuyOrSellError(error, reject);
                if (upbit.getOption('test')) {
    ...
    marketBuyOrSell(method) {
        return method.toUpperCase() === 'BUY' ? UpbitApi.marketBuy : UpbitApi.marketSell;
    },
    ...
    depthCache(tickers, limit=100, stagger=200) {
        return upbit.websockets.depthCacheStaggered(tickers, UpbitApi.sortDepthCache, limit, stagger);
    },

    sortDepthCache(ticker, depth) {
        depth.bids = upbit.sortBids(depth.bids);
        depth.asks = upbit.sortAsks(depth.asks);
    }
